<template>
  <div class="pagination">
    ...
    <!-- 第一个... -->
    <button disabled v-if="startEnd.start > 2" @click="switchPage(1)">...</button>
    <!-- 连续页面 -->
    <!-- eslint-plugin-vue -->
    <button v-for="num in startEnd.end" :key="num" v-if="num >= startEnd.start" :class="{active:num===currentPage}"  @click="switchPage(num)">
      {{ num }}
    </button>
    <!-- 第二个... -->
    <button disabled v-if="startEnd.end < lastPage - 1">...</button>
    <!-- 最后一页 -->
    <button v-if="startEnd.end !== lastPage"  @click="switchPage(lastPage)">{{ lastPage }}</button>
    <!-- 下一页按钮 -->
    <button

    ...
    <div class="modal-content" @click.stop>
      ...
      <div class="modal-footer" style="margin-left: 98px;">
        <button
          ...
          @click="$emit('confirm',addressInfo)"
        >确定</button>
...
export default {
  ...
  async mounted() {
    // 调用api接口函数,所有的省份
    const regionData = await this.$API.reqBaseRegion();
    // 获取所有的省份列表数据
  ...
  watch: {
    ...
    async "addressInfo.regionId"() {
      ...
      // 此时存在,请求的是当前区域中所有的城市
      const provinceData = await this.$API.reqAddressBaseProvince(
        this.addressInfo.regionId
      );
      // 获取了所有的城市了
